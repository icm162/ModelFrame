...
class GlaSDataset(Dataset):
    ...
    def split(self, index, split=[8, 2]):
        assert len(split) == 2, "数据集划分比例有误"
        summ, no = sum(split), 0 if(self.sub == "train") else 1 if(self.sub == "val") else -1
        return (index // split[no]) * summ + (index % split[no]) + (0 if(self.sub == "train") else split[0])

if(__name__ == "__main__"):
    """测试数据集是否能正常读取"""
    dataset = GlaSDataset(r"F:\DAT\GlaST", "train")
