...
from UNetSEG.Models.Components import D_CBR_ECA, D_BRC, BRC, BilinearInterpolation, SABlock

class UNetPPCTRECASA(nn.Module):

    model_name = "U++CES"

    def __init__(self, in_channels=3, num_classes=2, features=[32, 64, 128, 256], reveal=False, reveal_class=1) -> None:
        super(UNetPPCTRECASA, self).__init__()
...
if(__name__ == "__main__"):
    ...
    model = UNetPPCTRECASA(features=[32, 64, 128, 256])
    outs = model(x)
    print(outs.shape)
