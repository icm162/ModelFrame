...
from UNetSEG.Models.Components import D_CBR, D_BRC, BRC, BilinearInterpolation, ECABlock

class UNetPlusPlusCTR(nn.Module):

    model_name = "UNet++Concentration"

    def __init__(self, in_channels=3, num_classes=2, features=[32, 64, 128, 256]) -> None:
        super(UNetPlusPlusCTR, self).__init__()
        ...
        # 拼接输出 ECA
        self.out_dcbr = D_CBR(len(features) * num_classes, num_classes)
        self.eca = ECABlock(num_classes)
    ...
    def forward(self, x):
        ...
        # 输出层
        return self.eca(self.out_dcbr(catout))
        
        
if(__name__ == "__main__"):
    """模型测试"""
    x = torch.randn((1, 3, 512, 437))
    model = UNetPlusPlusCTR(features=[32, 64, 128, 256])
