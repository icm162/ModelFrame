...
from UNetSEG.Models.Components import Q_CBR_ECA, Q_DCBR_ECA, D_BRC, BRC,\
      BilinearInterpolation, SABlock, T5DSAG, ECABlock, SQPAM

class DPUPP(nn.Module):

    model_name = "DP-U-Net++"

    def __init__(self, in_channels=3, num_classes=2, layer=5, reveal=False, reveal_class=1) -> None:
        super(DPUPP, self).__init__()
        ...
        reverse_slope = nn.ModuleList()
        reverse_slope.append(nn.Sequential(Q_DCBR_ECA(in_channels, self.features[0]), self.downward))
        for i in range(layer - 5): 
            reverse_slope.append(nn.Sequential(Q_DCBR_ECA(self.features[i], self.features[i + 1]), self.downward))
        ...
        reverse_slope.append(nn.Sequential(self.o_model.layer4, SQPAM(self.features[-1]), ECABlock(self.features[-1])))
        self.nodes[0] = reverse_slope

        # 跳连注意力门 二维 左下至右上 左上至右下
        self.ags = nn.ModuleList()
        for j in range(len(self.features) - 1): 
            layer_ags = nn.ModuleList()
            for i in range(len(self.features) - 1 - j): layer_ags.append(T5DSAG(self.features[i + 1], self.features[i], self.features[i], self.features[i] // 2))
        ...
        self.out_init_cbr = Q_CBR_ECA((len(self.features) - 1) * num_classes, (len(self.features) - 1) * num_classes)
        self.out_pam = SQPAM((len(self.features) - 1) * num_classes)
        self.out_dcbr = Q_CBR_ECA((len(self.features) - 1) * num_classes * 2, num_classes)
    ...
    def forward(self, x):
        ...
        catout = self.out_init_cbr(catout)
        catout = torch.cat([catout, self.out_pam(catout)], dim=1)
