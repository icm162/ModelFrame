...
from UNetSEG.Models.Components import Q_CBR_ECA, D_BRC, BRC, BilinearInterpolation, SABlock, DSAG

class UPPDQCES(nn.Module):

    model_name = "U++DQCES"

    def __init__(self, in_channels=3, num_classes=2, features=[32, 64, 128, 256, 512], use_sa=True, reveal=False, reveal_class=1) -> None:
        super(UPPDQCES, self).__init__()
        ...
        for j in range(len(features) - 1): # 0 1 2 3
            ...
            for i in range(len(features) - 1 - j): layer_ags.append(DSAG(features[i + 1], features[i], features[i], features[i] // 2))
...
if(__name__ == "__main__"):
    ...
    model = UPPDQCES(features=[16, 32, 64, 128, 256, 512], use_sa=False)
    outs = model(x)
    print(outs.shape)
