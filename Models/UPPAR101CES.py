...
from UNetSEG.Models.Components import Q_CBR_ECA, D_BRC, BRC, BilinearInterpolation, SABlock, UAttentionGate

class UPPAR101CES(nn.Module):

    model_name = "UPPAR101CES"

    def __init__(self, in_channels=3, num_classes=2, reveal=False, reveal_class=1) -> None:
        super(UPPAR101CES, self).__init__()
        ...
        self.o_model = tv.models.resnet101(pretrained=True)
        ...
        self.first_tranchannel = nn.Conv2d(self.features[0], self.features[1], 1, 1, 0)
        reverse_slope.append(nn.Sequential(self.o_model.layer1, self.downward))
        reverse_slope.append(self.o_model.layer2)
        reverse_slope.append(self.o_model.layer3)
        reverse_slope.append(self.o_model.layer4)
        self.nodes[0] = reverse_slope

        # 跳联注意力门 二维 左下至右上 左上至右下
        self.ags = nn.ModuleList()
        for j in range(len(self.features) - 1): # 0 1 2 3
    ...
    def forward(self, x):
        ...
        for i, node in enumerate(self.nodes[0]): 
            ...
            elif(i == 1): val_nodes.append(F.resize(node(self.first_tranchannel(val_nodes[-1])), div2(val_nodes[-1].shape[2:])))
