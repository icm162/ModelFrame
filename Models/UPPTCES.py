...
from UNetSEG.Models.Components import T_CBR_ECA, D_BRC, BRC, BilinearInterpolation, SABlock

class UPPTCES(nn.Module):

    model_name = "U++TCES"

    def __init__(self, in_channels=3, num_classes=2, features=[32, 64, 128, 256], reveal=False, reveal_class=1) -> None:
        super(UPPTCES, self).__init__()
        ...
        # 编解码器连接层
        self.junction = T_CBR_ECA(features[-1], 2 * features[-1])

        # 编码器平行卷积块
        self.encoders = nn.ModuleList()
        for i, f in enumerate(features): self.encoders.append(T_CBR_ECA(in_channels if(i == 0) else features[i - 1], f))
        ...
        # 拼接输出 ECA
        self.sa = SABlock()
        self.out_dcbr = T_CBR_ECA(len(features) * num_classes, num_classes)
...
if(__name__ == "__main__"):
    ...
    model = UPPTCES(features=[32, 64, 128, 256])
    outs = model(x)
    print(outs.shape)
